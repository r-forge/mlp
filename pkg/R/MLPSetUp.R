...
###nMax = maximum number of genes in geneset.
  ...
  if(Org=="Mouse"){library(org.Mm.eg.db);go2entrez <- as.list(org.Mm.egGO2ALLEGS)}
  if(Org=="Human"){library(org.Hs.eg.db);go2entrez <- as.list(org.Hs.egGO2ALLEGS)}
  if(Org=="Rat"){library(org.Rn.eg.db);  go2entrez <- as.list(org.Rn.egGO2ALLEGS)}
