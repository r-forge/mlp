...
# f.rt, f.rtt, f.cmean, f.rmean, f.rsd,f.rsp,f.rspp, f.rvar, f.toarray, f.qnorm
# f.Vec2String,f.y2x, f.pp2g,
...
###f.u <- function(x){ f.cmeans(-log10(x)) }

f.u <- function(x){ f.cmean(-log10(x)) }
...
if(ncol(y)==2){return(cbind(n,as.vector(u)))} else {return(cbind(n,u))}
}


f.mlp <- function(x,y,y2x){
###This function calculates the mean of the gene-statistic y[,2] for each unique gene-set in x[,1].
###x is the input table of gene-sets and gene-names.
###y is the input table of gene-names and gene-statistics.
###y2x is the mapping of the gene-names in y to those in x.
...
###s1  <- split(y[y2x,-1],x[,1])
###u   <- t(sapply(s1,function(x){f.u(as.matrix(x))}))
s1  <- split(f.toarray(y[y2x,-1]),x[,1])
u  <- t(sapply(s1,function(x){f.u(matrix(x,ncol=ncol(y)-1))}))
###dimnames(u)[[2]] <- dimnames(s1[[1]])[[2]]
if(ncol(y)==2){return(cbind(n,as.vector(u)))} else {return(cbind(n,u))}
}
...
f.pperm <- function(x,p0,p1,ind.exact=1,np=0){
...
if (ind.exact==1){
   ...
   pj <- f.permtwo(n,n0)
   pj <- pj[-pmatch(f.Vec2String(c(p0,p1)),apply(pj,1,f.Vec2String)),]
   np <- nrow(pj)
   ...
   for (j in 1:np){
       pj <- sample(n)
       if( !is.na( pmatch(f.Vec2String(c(p0,p1)),f.Vec2String(pj)) )){pj <- sample(n)}
       j0 <- pj[1:n0]
       j1 <- pj[(n0+1):n]
       pp[,(j+1)] <- f.rt(x[,j0],x[,j1])[,4]
}}
...
f.Vec2String <- function(x){tmp0 <- NULL; for (j in 1:length(x)){tmp0 <- paste(tmp0,x[j],sep="")};return(tmp0)}
...
for (j in 1:n40){jj[j] <- (1:n41)[x41==x40[j]][1]}
...
###INDIVIDUAL CUTOFFS
w1 <- matrix( rep(w0[,2],ncol(w)),nrow(w0),ncol(w))
dw0w1 <-  ifelse(w1-w >0,1,0)
###pw0   <- 1-f.rmeans(dw0w1) 
pw0   <- 1-f.rmean(dw0w1) 
return(pw0)
}
...
work <- f.ee1(x=sqrt(w1[,1]), y=w1[,2], x0 = sqrt(w0[,1]), y0 = w0[,2],type=c("dec", "inc", "none")[3], m = 20, lqi = lqi, hqi=hqi, sym = F, plot = T, flag = F, dg = 15, logtran = F)
...
work <- f.ee1(x=sqrt(w1[,1]), y=w1[,2], x0 = sqrt(w0[,1]), y0 = w0[,2],type=c("dec", "inc", "none")[1], m = 20, lqi = lqi, hqi=hqi, sym = F, plot = T, flag = F, dg = 9, logtran = F)
pval <- f.ctpval1(work,nch=6)
rm(work)
return(pval)
}
...
f.ee1 = function(x,y,x0=x,y0=y,type= c("dec","inc","none")[3], m=20,lqi=0.05,hqi=0.95,sym=F,plot=T,flag=F,dg=15,logtran=F) {
...
if(type == "none") {
 if(length(qi) == 1) {xtp <- tt(xq2,qi); browser()} else {
 xtp <- tt(c(xq2[1,]),qi[1]); for(i in 2:length(qi))xtp <- cbind(xtp,tt(c(xq2[i,]),qi[i]))
   ...
   plot(x0,y0,xlab="n",ylab="MLP",axes=F,col="#08306B",pch=".")
   axis(2,lwd = 1.5,col="#08306B")
   temp.x <-(axis(1,labels=F))
   axis(1,lwd = 1.5,at=temp.x,labels=temp.x^2,las=1,col="#08306B")
   par(cex=0.5)
   npc=16; ccc="#2171B5"
   if (!is.null(hqi)) { i2 <- y0 > xtp[,length(qi)];print(length(i2[i2==TRUE]))
          points(x0[i2],y0[i2],col=ccc,pch=npc) 
          if (length(i2[i2==TRUE])>0){text(x0[i2],jitter(y0[i2],factor=4),labels=names(x0[i2]),cex=1.25,col="#08306B")}}
...
f.smdecreasing1 = function(z,w,decreasing=T) {
...
f.GOFiSH <- function(x,y,nmin=5,nmax=100,ind.sim=1,nsim=10,ind.smooth=1){

if(mode(x) != "numeric"){print("Input x should be numeric")
                         x <- f.toarray(x)
                        }
if(mode(y) != "numeric"){print("Input y should be numeric")
                         y <- f.toarray(y)
                        }
...
###Determine Cut-offs:
if (ind.smooth==1){
      if(ind.sim==0){print("Cannot smooth p-values if ind.sim=0")}
      ###pw0 <- f.cut1(w0[i,],w,q.cutoff=c(0.5,0.9,0.95,0.99,0.999,0.9999,0.99999))
      pw0 <- f.cut2(w0[i,],w,q.cutoff=c(0.5,0.9,0.95,0.99,0.999,0.9999,0.99999))
      } else {
      pw0 <- f.cut0(w0[i,],w)
      }

return(data.frame("Geneset.Size"=w0[i,1],"MLP.Observed"=w0[i,2],"P.Value"=pw0))
}


###===============================OUTPUT RETRIEVAL FUNCTIONS========================

f.GO2Name <- function(ANames,go.out,ind.p=1,p.cutoff=0.01){
if(ind.p==1){
ii <- row.names(go.out[go.out[,3]<= p.cutoff,])
} else{
ii <- row.names(go.out)
}
return(data.frame(go.out[ii,],"Geneset.Description"=I(ANames[ii])))
}
...
GO0 <- vector("list",lg0)
names(GO0) <- g0
for (j in 1:lg0){ GO0[[j]] <- A[A[,2]==g0[j],1]; names(GO0[[j]]) <- NULL}
return(GO0)
}
...
g0 <- vector("list",lG0)
names(g0) <- G0
for (j in 1:lG0){ g0[[j]] <- A[A[,1]==G0[j],2]; names(g0[[j]]) <- NULL}
return(g0)
}

f.G2P <- function(g0,G){
p0 <- NULL
for (j in g0){p0 <- c(p0,names(G[G==j]))}
return(p0)
}
...
f.GO2Gene <- function(x,y,go.out,go.pthreshold=0.01,gene.pthreshold=0.05,ind.log=0,fname="foo.pdf"){
